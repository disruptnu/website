...
export default function EventsLanding({ hamburgerMenuOpen, toggleHamburgerMenu }) {
      ...
      <div className="w-screen h-screen relative font-normal mb-20">
        <div className="w-screen h-20 flex items-start justify-between">
          ...
          <div className="flex items-center py-4 border border-solid rounded-full border-[#b5f727] mr-5 mt-5 hover:scale-110">
            <img src={hamburgerIcon} alt="hamburgerMenuIcon"
                 onClick={handleClickHamburgerMenu}
                 className="w-14 h-5 hover:cursor-pointer" />
          </div>
        </div>
        <div className="flex flex-col items-end pt-28 pb-20 w-8/12"
             style={{fontFamily: 'Faucet'}}>
          <p className="text-white text-4xl mb-1">
              disrupt
          </p>
          <p className="text-white text-9xl pb-8"
             style={{fontFeatureSettings: "'dlig' on, 'ss02' on, 'ss01' on"}}>
            EVENTS
          </p>
          <p className="text-white mx-auto ml-80 pl-48"
             style={{fontFamily: 'Usual-Regular'}}>
            Finnovate is an 8-week long program where students will be challenged to create a new innovative idea in the FinTech space that aims to disrupt the traditional financial service industry.
             Students will be formed into groups and attend a series of weekly workshops that will not only enhance their technical skills but their entrepreneurial business skills.
          </p>
        </div>
        <div className="flex">
