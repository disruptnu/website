    ...
    <div>
      <div className="border-t border-solid border-t-gray-400">
            <div className="mt-60 text-white font-bold">
                <p className="pl-10 text-xs mt-20"
                   style={{fontFamily: 'Usual-Light'}}>
                    / The Team
                </p>
                <p className="text-6xl text-left w-3/5 ml-10 mb-20 pt-20"
            ...
            <div>
                <div className="flex justify-start text-blue-500 text-lg w-screen h-5 pr-10 mb-10">
                    <img src={leftScroll}
                         ...
                         className={`hover:cursor-pointer mr-5 ml-10 ${CARD.length * 296 > window.innerWidth ? '' : 'hidden'}`}
                         onClick={scrollLeft}/>
                    <img src={rightScroll}
            ...
            <div className="flex flex-nowrap justify-start w-full h-3/4 text-2xl mt-4 mx-auto overflow-x-hidden divide-x divide-gray-800" ref={containerRef}>
                {CARD.map((card) => (
                <TeamShowcaseCard
                    key={card.key}
                    image={card.img}
